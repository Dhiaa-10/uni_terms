import json

with open("onboarding_raw.json", "r", encoding="utf-16") as f:
    data = json.load(f)

section = data.get("nodes", {}).get("17:4023", {}).get("document", {})

def find_check_full(node, results, parent_is_check=False):
    if node.get("name") == "CheckCircle":
        for child in node.get("children", []):
            find_check_full(child, results, True)
        return
    if parent_is_check and node.get("type") == "VECTOR":
        fills = []
        for f in node.get("fills", []):
            if f.get("type") == "SOLID" and "color" in f:
                c = f["color"]
                fills.append(f"#{int(c['r']*255):02x}{int(c['g']*255):02x}{int(c['b']*255):02x}")
        strokes = []
        for s in node.get("strokes", []):
            if s.get("type") == "SOLID" and "color" in s:
                c = s["color"]
                strokes.append(f"#{int(c['r']*255):02x}{int(c['g']*255):02x}{int(c['b']*255):02x}")
        results.append(f"  {node['name']}: fills={fills} strokes={strokes}")
    for child in node.get("children", []):
        find_check_full(child, results, parent_is_check)

first = section.get("children", [])[-1]
results = []
find_check_full(first, results)
with open("check_full.txt", "w", encoding="utf-8") as out:
    for r in results:
        out.write(r + "\n")
